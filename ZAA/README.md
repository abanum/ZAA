...
**▶ デモ: https://abanum.github.io/ZAA/**

（GitHub Pages を有効にすると上記 URL で直接動きます。`file://` と違い Secure Context になるため、画面キャプチャもそのまま使えます。）
...
- **オンラインで試す**: https://abanum.github.io/ZAA/
- **ローカルで開く**: `ntsc_snow.html`（単一 HTML 版）をブラウザで開く。分割された `index.html` は ES モジュールを使うため、`file://` で直接開くと動きません（下記のローカルサーバー経由で開いてください）。
...
> # http://localhost:8000/          （分割版 index.html）
> # http://localhost:8000/ntsc_snow.html （単一版）
> ```
...
| パス | 内容 |
|---|---|
| `index.html` | 分割版のエントリ（ES モジュールを読み込む。GitHub Pages / ローカルサーバー用） |
| `ntsc_snow.html` | 単一 HTML 版（全モジュールを結合済み。`file://` で直接開ける） |
| `src/` | ソース（1 ファイル 1 クラス、一方向依存） |
| `docs/` | 要件定義書（砂嵐・映像入力） |
| `tools/build_standalone.py` | `src/` を結合して `ntsc_snow.html` を再生成するスクリプト |
